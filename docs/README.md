# REM Documentation Index

Markdown is the authoritative documentation format for the current REM source.

- [User manual](userManual.md)
- [Architecture and transport contracts](architecture.md)
- [Runtime ownership](runtime-modules.md)
- [Android plugin system](plugins/android-plugin-system.md)
- [Android development and release builds](../apps/mobile/README.md)
- [1.2.7-rc.1 stabilization audit](stabilization/1.2.7-rc.1-audit.md)
- [1.2.7-rc.1 scenario results dashboard](stabilization/1.2.7-rc.1-results.html)
- [1.2.8 release notes](releases/1.2.8.md)
- [1.2.7 release notes](releases/1.2.7.md)
- [1.2.7-rc.1 prerelease notes](releases/1.2.7-rc.1.md)
- [Manual release gate](rem-1.2-manual-release-gate.md)

## Archived Manual Snapshots

The following files are retained for historical reference only. They are not
maintained, are not release evidence, and can contain obsolete instructions:

- `REM_User_Manual_IT_v1.0.6.pdf`
- `REM_User_Manual_IT_v2.docx`
- `REM_User_Manual_Plain_english_v3.pdf`
- `rem_manual_plain_english_v3.docx`
- `userManual.docx`

Do not regenerate or distribute these snapshots as the current manual. Use
`userManual.md` for REM `1.2.8`.
