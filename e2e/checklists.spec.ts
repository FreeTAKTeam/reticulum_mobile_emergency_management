...
test("operators can create a checklist and open its task detail", async ({ page }) => {
  ...
  await page.getByLabel("Checklist subtitle").fill("Metadata survives replication");
  ...
  await expect(openChecklist).toContainText("Metadata survives replication");
  await openChecklist.click();

  await expect(page).toHaveURL(/\/checklists\/chk-/);
