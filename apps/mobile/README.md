...
iOS compilation and CocoaPods are not release gates for `1.4`.
...
3. Set release-time metadata. `appVersionName` is `1.4` and
...
```bash
apksigner verify --verbose --print-certs path/to/rem-v1.4-release.apk
sha256sum path/to/rem-v1.4-release.apk path/to/rem-v1.4-release.aab
adb install -r path/to/rem-v1.4-release.apk
