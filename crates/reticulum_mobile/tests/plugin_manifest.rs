...
fn compile_host_callback_test_plugin_library(
    plugin_dir: &Path,
    metadata_id: &str,
    init_body: &str,
) -> PathBuf {
...
#[test]
fn native_host_storage_callbacks_are_permission_checked_and_callable() {
    let install_root = TestTempDir::new("native-loader-storage-callback-root");
    let plugin_dir = install_root.path().join("rem.plugin.example_status");
    fs::create_dir_all(plugin_dir.as_path()).expect("plugin dir exists");
    let library_path = compile_host_callback_test_plugin_library(
        ...
        "rem.plugin.example_status",
        r#"
    let storage_set = host.storage_set.expect("storage_set callback");
    let storage_get = host.storage_get.expect("storage_get callback");
    let free_buffer = host.free_buffer.expect("free_buffer callback");
    let key = b"counter\0".as_ptr() as *const c_char;
    let value = b"{\"value\":5}\0".as_ptr() as *const c_char;
    if unsafe { storage_set(host.ctx, key, value) } != 0 {
        return 1;
    }
    let mut out = RemPluginHostBuffer {
        ptr: std::ptr::null_mut(),
        len: 0,
    };
    if unsafe { storage_get(host.ctx, key, &mut out) } != 0 {
        return 1;
    }
    ...
    let ok = bytes == b"{\"value\":5}";
    unsafe { free_buffer(host.ctx, out) };
    if ok { 0 } else { 1 }
"#,
    );
    write_dynamic_plugin_manifest(plugin_dir.as_path(), library_path.as_path());
    ...
    let host = PluginHostApi::new(registry);

    let mut plugin = NativePluginLibrary::load(candidate).expect("native plugin loads");
    plugin
        .initialize_with_host_api("rem.plugin.example_status", host)
        .expect("plugin storage callback succeeds");
}

#[test]
fn native_host_send_lxmf_callback_queues_outbound_request() {
    let install_root = TestTempDir::new("native-loader-send-callback-root");
    let plugin_dir = install_root.path().join("rem.plugin.example_status");
    fs::create_dir_all(plugin_dir.as_path()).expect("plugin dir exists");
    let library_path = compile_host_callback_test_plugin_library(
        plugin_dir.as_path(),
    ...
    let request = b"{\"destinationHex\":\"aabbccddeeff00112233445566778899\",\"messageName\":\"status_test\",\"payload\":{\"status\":\"ok\"},\"bodyUtf8\":\"Status test from native callback\",\"title\":\"Status Test\",\"sendMode\":{\"PropagationOnly\":{}}}\0".as_ptr() as *const c_char;
...
"#,
    ...
    plugin
        .initialize_with_host_api("rem.plugin.example_status", host)
        .expect("plugin send callback succeeds");

    let queued = plugin.drain_queued_lxmf_outbound_requests();
    assert_eq!(queued.len(), 1);
    assert_eq!(
        queued[0].destination_hex.as_str(),
        "aabbccddeeff00112233445566778899"
    );
    assert_eq!(queued[0].message_name.as_str(), "status_test");
    assert_eq!(
        queued[0].body_utf8.as_str(),
        "Status test from native callback"
    );
